...
Permite ao usuário gerenciar múltiplas fichas de treino. Cada ficha pode ser ativada/desativada, tem nome automático ou personalizado, duração configurável com alerta de vencimento, e descrição por dia de treino.
...
| Tela | `src/screens/Workout/WorkoutDayScreen/` | Descrição por dia de treino |
...
2. **Trocar ficha ativa:** WorkoutPlans → switch ligado → clicar na ficha desejada
3. **Ver detalhes:** WorkoutPlans → clicar na ficha → modal com treinos e exercícios → opção de ativar
4. **Vencimento:** Home detecta ficha vencida → alerta "Sua ficha venceu" → modal para renovar ou dispensar
