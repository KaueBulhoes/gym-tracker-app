...
- **Redirect:** define `emailRedirectTo` para `gymtracker://login` no e-mail de confirmação
- **Tabelas:** `auth.users`
...
- **O que faz:** Processa payloads de auth no deep link e retorna `true` apenas para fluxo de recuperação (`type=recovery`)
...
- Confirmação de e-mail usa redirect explícito: `gymtracker://login`
